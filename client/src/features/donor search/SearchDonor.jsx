...
// ==========================================
// 1. HELPERS & MATH (Distance Calculation)
// ==========================================
const MOCK_BLOOD_GROUPS = ['A+', 'A-', 'B+', 'B-', 'O+', 'O-', 'AB+', 'AB-'];

// Format blood type for the small badge (e.g., "A+" -> "A POS")
const formatGroupLabel = (bloodType) => {
  if (!bloodType) return '';
  return bloodType.replace('+', ' POS').replace('-', ' NEG');
};

// Haversine formula to calculate exact distance between two coordinates in km
...
// ==========================================
// 3. DONOR CARD (With Directions Routing)
// ==========================================
const DonorCard = ({ donor, userLocation }) => {
  // 1. Clean the phone number for standard URI protocols
  const cleanPhone = donor.phone?.replace(/[^0-9+]/g, '') || '';

  // 2. Generate Google Maps Directions Link
  // If we have the user's location, route FROM them TO the donor.
  // If not, just open Maps with the donor as the destination.
  let mapUrl = `https://www.google.com/maps/dir/?api=1&destination=${donor.lat},${donor.lng}`;
  if (userLocation && userLocation.lat && userLocation.lng) {
    mapUrl += `&origin=${userLocation.lat},${userLocation.lng}`;
  }

  // 3. Calculate distance if user location is available
  ...
  return (
    <div className="flex flex-col justify-between rounded-2xl border border-gray-50 bg-white p-4 shadow-[0_2px_10px_rgba(0,0,0,0.04)] md:p-5">
            ...
            <div>
              ...
              <div className="mt-1 flex flex-col gap-1 text-xs text-gray-500 md:flex-row md:items-center md:gap-2 md:text-sm">
                <span className="flex items-center gap-1 font-medium">
                  ...
                  {/* Show exact distance if available, otherwise fallback */}
                  {exactDistance
      ...
      {/* --- Action Buttons (Map, WhatsApp, Call) --- */}
...
export default function SearchDonor() {
  ...
  // 1. Fetch User Location & API Data on Mount
  useEffect(() => {
    // Get user's current location to calculate distances & map routes
    if (navigator.geolocation) {
      navigator.geolocation.getCurrentPosition(
        ...
        (err) =>
          console.log(
            'User denied location, distances will show as unknown.',
            err
          )
      );
    }
      ...
      try {
        ...
        const mappedDonors = response.data.data.users.map((user) => {
          ...
          return {
            ...
            lat: lat,
            lng: lng,
            // Fallback dummy phone if API doesn't have one
