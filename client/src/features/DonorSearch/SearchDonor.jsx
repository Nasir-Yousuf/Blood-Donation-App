      ...
      <div className="rounded-[20px] border border-gray-100 bg-[#F8F9FA] p-6 shadow-sm">
        ...
        <div className="mb-6">
          ...
          <div className="mt-2 flex justify-between text-xs text-gray-400">
            <span>1 km</span>
            <span>500 km</span> {/* 👇 FIXED: Label is now 500km */}
          </div>
        </div>
...
export default function SearchDonor() {
  ...
  const [distance, setDistance] = useState(500); // 👇 FIXED: Default 500km
  ...
  // 2. Fetch Logic (Triggers when applyTrigger changes OR page changes)
  useEffect(() => {
    if (userLocation === null) return;

    const fetchDonors = async () => {
      if (page === 1) setIsLoading(true); // Only show full loading spinner on initial fetch/filter
      setError(null);

      try {
        let endpoint = '/users/';
        let params = { page: page, limit: 20 }; // Pass pagination limits
        ...
        const mappedDonors = rawData.map((user) => {
          ...
          return {
            ...
            tags: [
              ...
              { text: user.role?.toUpperCase() || 'DONOR', type: 'blue' },
            ],
          };
        });

        // Sort by exact calculated distance on the frontend if we have location
        if (userLocation) {
        ...
        if (page === 1) {
          ...
          setDonors((prev) => [...prev, ...mappedDonors]);
        }
      } catch (err) {
    ...
    fetchDonors();
  }, [applyTrigger, page, userLocation]);

  const handleApplyFilters = () => {
    setPage(1); // Reset back to page 1 on new filter
    setDonors([]); // Clear UI immediately
    setApplyTrigger((prev) => prev + 1);
  };
          ...
          <div className="flex-1">
            ...
            {isLoading && page === 1 ? (
              ...
              <>
                ...
                {/* 👇 LOAD MORE BUTTON (Restored with your exact styling framework) */}
                {hasMore && (
                  <div className="mt-10 flex justify-center pb-8">
                    <button
                      onClick={() => setPage((prev) => prev + 1)}
                      className="rounded-xl border border-gray-200 bg-white px-8 py-3.5 text-sm font-bold text-gray-700 shadow-sm transition-all hover:border-gray-300 hover:bg-gray-50 hover:text-[#D32F2F]"
                    >
                      Load More Donors
                    </button>
                  </div>
                ...
                {!hasMore && donors.length > 0 && (
                  <div className="mt-10 pb-8 text-center text-sm font-bold text-gray-400">
